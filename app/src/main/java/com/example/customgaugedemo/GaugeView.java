...
public class GaugeView extends View {
    ...
    Paint needleLeftPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    Paint needleRightPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    Paint needleCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void init() {
        ...
        progressPaint.setColor(Color.parseColor("#3498db"));
        progressPaint.setStrokeWidth(0.03f);
    }
...
//				new float[]{0.60f, 0.85f, 0.96f, 0.99f}, TileMode.MIRROR));
    ...
    public Paint getDefaultNeedleLeftPaint() {
        needleLeftPaint.setColor(ContextCompat.getColor(mContext, R.color.blue));
        return needleLeftPaint;
    }

    public Paint getDefaultNeedleRightPaint() {

        needleRightPaint.setColor(ContextCompat.getColor(mContext, R.color.blue));
        //paint.setShadowLayer(0.01f, 0.005f, -0.005f, Color.argb(127, 0, 0, 0));
        return needleRightPaint;
    }
    ...
    public Paint getDefaultNeedleScrewPaint() {
        ...
        needleCirclePaint.setColor(ContextCompat.getColor(mContext, R.color.blue));
        return needleCirclePaint;
    }

    public void setNeedleColor(int needleColor) {
        needleLeftPaint.setColor(needleColor);
        needleRightPaint.setColor(needleColor);
...
//        }
    ...
    private void drawScale(final Canvas canvas) {
        ...
        for (int i = 0; i < totalTicks; i++) {
            ...
            final Paint thresholdPaint = new Paint(Paint.LINEAR_TEXT_FLAG | Paint.ANTI_ALIAS_FLAG);
...
//            if ((Math.abs(mod - 0) < 0.001) || (Math.abs(mod - div) < 0.001)) {
            ...
            paint.setColor(Color.WHITE);
            thresholdPaint.setStrokeWidth(0.01f);
            thresholdPaint.setColor(Color.RED);

            canvas.drawLine(0.5f, y1 - 0.015f, 0.5f, y3 - 0.07f, paint);
            if(value == 80F) {
                canvas.drawLine(0.5f, y1 - 0.015f, 0.5f, y3 - 0.07f, thresholdPaint);
            }
...
//            paint.setStrokeWidth(0.03f);
            ...
            if (angle != 0 && angle != 240) {
                ...
                canvas.drawArc(mDynamicBorderRect, 148, sweepAngle, false, progressPaint);
